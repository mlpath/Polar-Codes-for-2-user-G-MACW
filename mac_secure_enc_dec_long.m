...
for m = 1:length(block_length)
    ...
    for l = 1:nL(m):L
        ...
        for j = 1:nsamp
            ...
            %
            fprintf('block length %d, iteration %d, rate %3.1f percent, Err_b %d, Err_e %d \n', block_length(m), j, 100*sum(any(new_grid,1))/sum(any(user_grid,1)), Err_b, Err_e);
            %     for i = 1:t
    ...
    output_str = strcat('error_rate_',num2str(block_length(m)),'.mat');
