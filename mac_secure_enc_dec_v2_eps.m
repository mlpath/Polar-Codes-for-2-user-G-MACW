...
for m = 1:length(power_id)
    filename_str = strcat('secrecy_graph_',num2str(power_id(m)),'.xlsx');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %                       Input from find_secure.m
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % by default we start from column A1 and the code length is N = 256
    [rate_vectors, user_grid, eve_rates] = find_secure_v3(filename_str, Epsilon, 256, 1, 2);
    %
    r = rate_vectors(:,end);
    % for debug purpose
    ...
    N = size(user_grid, 2);
    % for debug
    [rate_vectors(time_slots,:), eve_rates(time_slots, :)]
    time_slots'
    %
    ...
    Err_e = 0;
    %
    block_error_b = zeros(1,length(time_slots(1:nL:L)));
    block_error_e = block_error_b;
    %
    rates = zeros(1,length(time_slots(1:nL:L)));
    %
    ...
    for l = 1:nL:L
        if (l + nL) <= L
            ...
            new_grid = user_grid;
            new_grid(:,time_slots(l+nL:end)) = 0;
        else
            new_grid = user_grid;
        end
        ...
        for j = 1:nsamp
            ...
            %
            fprintf('power %d, iteration %d, rate %3.1f percent, Err_b %d, Err_e %d \n', power_id(m), j, 100*sum(any(new_grid,1))/sum(any(user_grid,1)), Err_b, Err_e);
            %     for i = 1:t
        ...
        block_error_b(ceil(l/nL)) = Err_b/nsamp;
        block_error_e(ceil(l/nL)) = Err_e/nsamp;
        %
    ...
    output_str = strcat('error_rate_eps',num2str(power_id(m)),'.mat');
    % to make sure we don't lose any result, also save them
    save(output_str, 'rates','block_error_b','block_error_e', '-v6');
end
%
