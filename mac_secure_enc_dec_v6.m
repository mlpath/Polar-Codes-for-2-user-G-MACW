...
for m = 1:length(power_id)
    ...
    [rate_vectors, user_grid, ~] = find_secure_v2(filename_str, Epsilon, 256, 1, 2);
    %
    r = rate_vectors(:,end);
    % for debug purpose
...
%     Err_e = 0;
    ...
    %
    if power_id(m) == 10
    % number of channels we transmit on at each iteration
        nL = [5,7,10];
    elseif power_id(m) == 15
        nL = [5, 7, 11];
    elseif power_id(m) == 20
        nL = [2 5];
    end
    % 
    block_error_b = zeros(1,length(nL));
    rates = zeros(1,length(nL));
    %              
    ...
    for l = 1:length(nL)
        if (1 + nL(l)) <= L
            ...
            new_grid = user_grid;
            new_grid(:,time_slots((nL(l)+1):end)) = 0;
        else
            new_grid = user_grid;
        end
...
%             end
            %
            fprintf('power %d, iteration %d, rate %3.1f percent, Err_b %d \n', power_id(m), j, 100*sum(any(new_grid,1))/sum(any(user_grid,1)), Err_b);
            %     for i = 1:t
...
%     hold on;
    ...
    output_str = strcat('error_rate_b_',num2str(power_id(m)),'_70','.mat');
    % to make sure we don't lose any result, also save them
    save(output_str, 'rates','block_error_b','-v6');
end
%
