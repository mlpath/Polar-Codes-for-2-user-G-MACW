...
% function [secrecy_rate, ts_secure, b_bound, val_min, e_bound] = batacharya_mac_v4(epsilon, eta, excel_id)
tic
...
if strcmp(excel_id, '1024') || strcmp(excel_id, '512') || strcmp(excel_id,'256')
    block_length = str2double(excel_id);
    chan_id = '_p10';
else
    block_length = 256;
%     chan_id = strcat('_p', excel_id);
    chan_id = '';
end
%
[~, user_grid, out_matrix] = find_secure_v22(filename_str, epsilon, block_length, 1, 2);
%
...
for i = 1:length(ts_secure)
    ...
    table_id = dec2bin(ts_secure(i),log2(block_length));
    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     if exist(test_str, 'file') == 0
        ...
        su_prob_v2('debug_channels_4_p20', table_id, chan_id, 'bob');
        %     end
        % this loads Z parameters
        load su_pars_b
        Z1 = z_par;
        %
        su_prob_v2('debug_channels_8_10_p20', table_id, chan_id, 'eve');
        load su_pars_e
        Z2 = z_par;
...
[~, ix_sort] = sort(bound(:,2),'ascend');
% reorder values
bound = bound(ix_sort,:);
L2 = L2(ix_sort);
ts_secure_old = ts_secure;
ts_secure = ts_secure(ix_sort);
isequal(length(intersect(ts_secure, ts_secure_old)),length(ts_secure))
%
for i = 1:length(ts_secure)
    tmp = sum(bound(1:i,2));
    if tmp > eta 
...
% upper bound
%
% only if we are not in outage (i.e. ix_cum has a value other than zero)
if ix_cum
...
%     secrecy_rate = ix_cum/block_length;%(length(ts_secure) - count)/256;
    %
    size(ts_secure)
    block_length
    ...
    % 
    b_bound(2) = sum(bound(1:ix_cum,2));
    % ix is the index of subchannels with highest error bound
    [b_bound(1), ix] = max(bound(1:ix_cum,1));
    [e_bound, ~] = max(L2(1:ix_cum));
    % and print it too
%     fprintf('\nlower and upper bound on block error probability (adjusted): [%1.2e, %1.2e] \n',b_bound(1), b_bound(2));
    %
    [val_min, ix_min] = min(L2(1:ix_cum));
    [val_max, ix_max] = max(L2(1:ix_cum));
    %
...
% In an ideal world, I expect that we can find some subchannels (i.e. having
