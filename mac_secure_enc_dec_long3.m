...
%             end
            %
            fprintf('block length %d, iteration %d, rate %3.1f percent, Err_b %d \n', block_length(m), j, 100*sum(any(new_grid,1))/sum(any(user_grid,1)), Err_b);
            %     for i = 1:t
...
%     hold on;
    ...
    output_str = strcat('error_rate_b_',num2str(block_length(m)), '_2','.mat');
